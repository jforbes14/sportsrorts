...
My thinking is that if the Liberal party allocated grants to electorates (seats) on the basis of their voting behaviour, then this strategy could be revealed with an appropriate statistical model.
...
The ultimate aim of my analysis is to see whether there is evidence that grants were allocated on the basis of electorates' voting behaviour from the previous elecction. Essentially, is there an underlying strategy? And if so, what voting pattern was targeted by the Liberals?
...
Wow! The loess curve is reveals a pretty significant relationship - one that was not obvious from inspecting the initial scatterplot. 

There is a clear spike in the amount of grant money given to electorates that were won by the Liberal party in 2016, but not won by a huge margin. These seats are called "marginal" Liberal party seats. This plot in itself is pretty convincing evidence of Liberal bias in the funding scheme.

But to take this another step further, I decided to fit a Generalized Additive Model (GAM) with the grant amount as the response, and the two-party preferred vote, swing vote, and Census characteristics as covariates. Each of covariate is included in the model as a smooth term. In this case I have used penalized regression splines - a type of flexible non-parametric regression similar to loess.
...
It can be clearly seen that even after accounting for the swing vote and Census variables, there is still a huge spike in grant amount for marginal Liberal party seats! The p-values from the GAM regression output confirm that this is the conclusive evidence I set out to discover. With a p-value < 0.001, the data proves that the two-party preferred vote had a statistically signficant effect on total grant money allocated to each electoate.

Finally, to check that my findings are robust to outliers, I re-estimate the model after removing the two electorates with outstandingly large grant amounts ($1.8M and $2.0M - the electorates of Boothby and Dawson), and find that the same Liberal bias is observed. Additionally, the model assumptions seem to be valid, as the residuals are approximately normal (observed using a quantile-quantile plot). Also for those wondering, the fully specified GAM explains 46.3% of the deviance - which I consider to be a pretty good fit in this case.

#### In conclusion

My statistical model provides empirical evidence that the Liberal party did allocate grants with a biased strategy to beef up support for the 2019 election. The data shows that marginal Liberal seats were targeted by the government and received more grant funds as a result. 

This is a conclusive finding in the context of Australian politics, but more importantly, this analysis demonstrates how readily powerful insights can be found using publicly available data. I hope you, as a fellow data scientist, feel empowered by this.
